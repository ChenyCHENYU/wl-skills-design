...
- **优点**：
  ...
  - 已发布设计域统一拥有「空白模板 + 真实样例」，每层都能朝最佳实践收敛。
- **代价**：
...
## ADR-014 · 引入变更影响分析，作为设计协同引擎第一层能力
...
此前各 Skill 主要解决“从零生成”和“单域验证”。真实项目中更高频的是增量设计：字段、状态、接口或页面变化后，需要同步 spec、词典、DB、API、原型和评审矩阵。若继续依赖人工记忆同步，最容易出现字段追溯断裂、状态机不一致、接口和原型脱节。

### 决策

1. 新增规范 `standards/09-change-impact.md`，定义变更登记、影响传播规则、P0/P1/P2 等级、补丁任务格式和 CI-A/B/C/X 20 项验证清单。
2. 新增 `cross/change-impact/` Skill，定位为“先分析影响，再排补丁，再复验”，不直接替代单域 Skill。
3. 新增 `analyze-change-impact` / `validate-change-impact` prompt，把变更分析纳入可执行闭环。
4. manifest 注册 `cross.changeImpact`，使“改字段影响哪些文档”“新增状态要同步什么”等请求精准路由到该 Skill，而非误判为单域 DB/API 设计。
5. 默认补丁顺序为 `glossary → spec → DB → API → prototype → design-review`，但允许根据变更来源调整。

### 权衡

- **优点**：把设计体系从“生成器”推进到“协同引擎”；变更影响先显式化，再由单域 Skill 精准修复；P0/P1 补丁任务和复验入口可追踪。
- **代价**：本阶段仍只输出补丁计划，不自动批量改写所有产物；这是有意控制风险，避免 AI 在影响未确认前跨多文档盲改。
