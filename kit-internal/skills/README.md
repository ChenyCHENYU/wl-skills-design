...
|-------|-------|------|---------|---------|
| 术语/字段词典（统一语言） | 🔴 高 | cross | 08-glossary.md（待写） | 规范 + SKILL.md + 模板；作为 spec/DB/接口字段对齐的中央锚点，反哺 DB-X / IF-X / D4 校验 |
...
> **词典优先级最高的原因**：当前 spec↔DB↔接口的字段对齐靠「中文名匹配 / 英文名子集」，
> 缺一个中央词典作为锚点。补 glossary 后，字段对不齐从「评审时发现」提前到「设计时杜绝」，
> 直接增强已有的三角联动校验。建议作为下一个主版本（v1.0）的核心。
