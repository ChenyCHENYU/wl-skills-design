...
## [0.11.1] — 2026-08-15（快速上手文档定版）

### 修复与变更

- README「快速开始」重排为三场景上手（从零到一 / 接入半成品 / 机械验证），含可直接复制的示例指令、三层输入节奏与日常管理速查。
- 包内使用指南（安装后 `.github/guides/usage.md`）同步增加「快速上手」章节，并接入输入准备清单与四域 verify 示例。
...
- 需求说明书标准新增 GB 颗粒度基线（§5.1 按钮级覆盖 + §5.2 GB1–GB8）：命令按钮处理逻辑按编号步骤书写，覆盖数据对象、字典稳定值、前置校验、审计履历、联动外同步、异常文案、展示规则与步骤可测性；执行类活动与命令按钮一一对应。
- 新增匿名合成样例 `requirements-spec-doc/examples/05-ipo-granularity.md` 作为步骤级 IPO 对照。
- 功能编码支持可选子域码 `[模块码][子域码][NNN]`，适配 10+ 功能的二级菜单模块。
- CLI 新增 `restore --list` / `restore --id <backupId>` / `uninstall --purge`。
- design-model 校验新增 DM014–DM018：必填字段缺失、未定义字段、`nameEn`/`operationId` 命名、`source.path` 约束，逐码补齐测试。

### 修复与安全

- restore 覆盖现存文件前生成安全快照，恢复操作本身可再次 restore 撤销。
- 写入操作增加锁文件（5 分钟过期自动清理），防止并发安装互相破坏；状态写入去除删除窗口，全程原子替换。
- 状态文件校验 package 与 schemaVersion；升级遇到已停用编辑器 profile 时自动降级并告警，不再直接失败。
- doctor 隐私扫描改为去分隔符归一化匹配，堵住分隔符绕过；清理 `03-database.md` 历史绕过写法，评审样例编码改为 `DEMO-OM`。
- 调度正文意图链补齐 `maintain`，并新增与 `manifest.intentPriority` 的一致性门禁；Prompt 数量改由 manifest 派生断言。
- 路由排序实际使用 manifest `priority` 作为同分决胜；意图识别新增 审查/走查 同义词；路由语料扩至 31 条，新增歧义并列（ask-one-question）与纯负例。
- `package-smoke.js` 升级为真实 npm 载荷冒烟：pack 白名单/黑名单断言 + 全新目录安装 + 安装后 CLI 全命令闭环。
