...
| 部分 | 值 | 含义 |
|------|----|------|
| 流程编码 | `PMMB-A-02` | 物料主数据子模块第 2 个流程（spec §十.2）|
| 操作类型 | `E` | 金恒平台操作 |
| 活动编号 | `01` | 该流程第 1 个活动 |
...
> 编码层（① 层）显示被引流程的编码（如 `PMMB-A-02`），岗位层（③ 层）显示执行部门。  
> 活动编码格式：`[流程编码]-[操作类型]-[NN]`（spec §十.3），如 `PMMB-A-02-E-01`。
...
- [ ] 14. 活动编码格式符合 spec §十.3：`[流程编码]-[操作类型]-[NN]`（如 `PMMB-A-02-E-01`）
- [ ] 15. 模块色标使用正确，无串色

**跨文档一致性（5项，与 spec 文档联动时执行）**
- [ ] 16. **[FC-01]** 流程图中每个活动节点的活动编码，与对应 spec 文档 4.x.2 活动说明表中的编码完全一致（无多余编码、无遗漏编码）
- [ ] 17. **[FC-02]** 流程图所有泳道（岗位/部门名称），与 spec 文档 2.2 岗位定义表中的岗位名称完全匹配（字面完全相同）
- [ ] 18. **[FC-03]** 流程图系统活动节点数量，与 spec 4.x.3 对照表中非「线下操作」的行数一致
- [ ] 19. **[FC-04]** 流程编码格式符合 `[MODULE]-A-[NN]`（如 PMPM-A-01），无自定义格式
- [ ] 20. **[FC-05]** 同一流程内活动编码从 E-01 起连续递增，无跳号、无重复
