...
| 模式代码 | 模式名称 | 典型场景（钢铁行业） | 布局结构 |
|---------|---------|-------------------|---------|
| `LIST` | 标准列表页 | 炼钢计划列表、订单台账、质检记录 | 查询区 + 工具栏 + 表格 + 分页 |
| `FORM_MODAL` | 弹窗表单 | 新增/编辑计划、审批弹窗 | 弹窗 + 表单分区 + 按钮组 |
| `FORM_ROUTE` | 路由表单 | 复杂新增（多步骤）、详情页 | 独立页面 + 表单分区 + 步骤条（可选） |
| `MASTER_DETAIL` | 主从表 | 轧钢计划+钢坯明细、订单+订单行 | 上下分栏（jh-drag-row） |
| `TREE_LIST` | 树形+列表 | 组织架构+人员、设备分类+设备 | 左右分割（C_Splitter + C_Tree） |
| `TAB_FORM` | 多 Tab 表单 | 基础资料维护（多维度属性） | Tab 切换 + 各 Tab 内表单/表格 |
| `DASHBOARD` | 仪表盘 | 生产看板、产能利用率 | 卡片 + 图表 + 统计指标 |
| `COMPOSITE` | 组合页面 | 炼钢计划编制（查询+组炉+明细） | 多模式组合，需逐区标注 |
...
- **目录名**：kebab-case（如 steel-rolling-plan）
- **服务缩写**：模块缩写（如 produce / sale / quality）
- **所属流程**：关联的系统流程编号（如 PMPM-A-01）
- **关联 IPO**：spec 中对应的功能编码（如 PMPM007）
```
...
| 必须标注项 | 说明 | 示例 |
|-----------|------|------|
| 字段名（camelCase） | 与接口提交字段对应 | `steelGrade` |
| 中文标签 | 表单 label | 钢种 |
| 组件类型 | input / select / date / number / textarea / ... | `select` |
| 是否必填 | 表单校验 | 是 |
| 字典编码 | 下拉选项来源 | `steel_grade` |
| 默认值 | 有则标注 | — |
| 联动规则 | 字段间联动（可选） | 选择钢种后自动带出合金配比 |
...
| 层级 | 命名格式 | 示例 |
|------|---------|------|
| 原型标注 | camelCase | `steelGrade` |
| spec IPO | 中文名 | 钢种 |
| 数据库 | snake_case | `steel_grade` |
| 接口 | camelCase | `steelGrade` |
...
spec 功能编码  →  原型页面标注  →  prototype-scan  →  代码目录
  PMPM007          PMPM007 炼钢计划编制      page-spec        steel-plan/
```

| 来源 | 编码 | 示例 |
|------|------|------|
| spec 功能编码（权威） | `[子模块代码][NNN]` | `PMPM007` |
| 原型页面元信息「关联 IPO」 | 引用 spec 功能编码 | `PMPM007` |
...
示例：`views/produce/production-plan/steel-rolling-plan/`
...
> 以下为钢铁行业（冶金/轧钢/质检/物流）项目的特殊约定，其他行业可忽略。

### 7.1 常见页面模式映射

| 业务场景 | 推荐模式 | 说明 |
|---------|---------|------|
| 基础资料维护（产率/BOM/工作天） | LIST + FORM_MODAL | 标准 CRUD |
| 计划编制（炼钢/轧钢/精整） | COMPOSITE | 查询+组炉/组浇+明细 |
| 订单管理（合单/挂单/拆单） | MASTER_DETAIL | 订单主表+订单行 |
| 推估试算（年度/月度） | TAB_FORM | 多工序 Tab 切换 |
| 质检记录 | LIST + FORM_ROUTE | 列表+复杂录入表单 |
| 生产看板 | DASHBOARD | 实时数据+图表 |
| 设备台账 | TREE_LIST | 设备分类树+设备列表 |

### 7.2 钢铁行业高频字典

| 字典编码 | 用途 | 典型值 |
|---------|------|--------|
| `steel_grade` | 钢种 | Q235B / HRB400E / ... |
| `process_line` | 产线 | 炼钢 / 轧钢(Bar) / 精整酸洗 |
| `plan_status` | 计划状态 | 待排产/已排产/执行中/已完成 |
| `order_status` | 订单状态 | 待评估/已确认/生产中/已交付 |
| `equipment_type` | 设备类型 | 电弧炉/连铸机/轧机/... |

### 7.3 钢铁行业特殊交互

| 交互 | 说明 | 标注要求 |
|------|------|---------|
| 组炉/组浇 | 多条钢坯需求合并为一炉 | 标注勾选规则+重量/支数汇总逻辑 |
| 炉批追溯 | 从成品追溯到炉号 | 标注跳转链路 |
| 产能约束 | 排产时校验产能上限 | 标注约束提示方式 |
| 多倍坯计算 | 根据成品规格计算坯料 | 标注计算触发时机 |
...
{
  "pageName": "炼钢计划列表",
  "kebabName": "smelting-plan",
  "pattern": "LIST",
  "path": "views/produce/production-plan/smelting-plan/",
  "query": [...],
  "toolbar": [...],
  "columns": [...],
  "operations": [...]
...
当原型设计 Skill 正式发布后，集成评审（07-design-review）的 D4 维度将新增：
- 原型字段 ⊆ spec IPO 字段
- 原型字典 = DB 枚举
- 原型按钮 → 接口覆盖
