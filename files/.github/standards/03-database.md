...
本文件是**数据库设计的唯一权威来源**，约定：库表命名、强制系统字段、主键与索引、文档结构、数据字典格式、变更设计、与需求说明书（spec）的字段联动、验证清单（34 项）与闭环修复协议。

> 配套技能：`.github/skills/data/database/SKILL.md`（操作流程） + `.github/prompts/create-db-design.prompt.md`（生成） + `validate-db-design.prompt.md`（验证）。
> **规范定义"做什么"，Skill 定义"怎么做"，两者不重复。**

设计哲学（沿用真实项目数据字典 10 列结构，叠加最佳实践）：

```
真实文档基线（保留）         本规范叠加（最佳实践）
──────────────────         ──────────────────────
ER 图 + DB 清单 + 数据字典    + 7 个强制系统字段
...
> 示例：`pmom_order_main`（生产-订单-订单主表）、`pmpm_plan_detail`（生产-计划-计划明细表）。
...
## §二  强制系统字段（7 项，每张业务表必备）

> 真实文档基线缺失系统字段——本规范强制补齐。除纯字典/配置表外，每张业务表**必须**包含以下 7 个字段，统一置于字段清单末尾（审计区）。

| 序号 | 字段英文名 | 字段中文名 | 类型 | 说明 |
|------|-----------|-----------|------|------|
| S1 | `id` | 主键 | bigint / varchar(32) | 主键，雪花 ID 或 UUID，见 §三 |
| S2 | `createdBy` | 创建人 | varchar(32) | 记录创建者用户 ID |
| S3 | `createdTime` | 创建时间 | datetime | 默认 `CURRENT_TIMESTAMP` |
| S4 | `updatedBy` | 更新人 | varchar(32) | 最后更新者用户 ID |
| S5 | `updatedTime` | 更新时间 | datetime | 更新时自动刷新 |
| S6 | `deletedFlag` | 删除标志 | tinyint | 0=正常 1=已删除（逻辑删除）|
| S7 | `tenantId` | 租户号 | varchar(32) | 多租户隔离；单租户系统可标注"暂留空" |

> **逻辑删除强制**：业务表禁止物理删除，统一用 `deletedFlag`。所有查询默认带 `deletedFlag = 0` 条件。
...
> 沿用真实项目结构：一个数据库分册按**模块**组织，每个模块固定输出以下 4 节，顺序不可变。
...
|------|------|---------|--------|------|---------|
| 1 | `pmom_order_main` | 订单主表 | 主档 | 生产订单头信息 | 万级 |
| 2 | `pmom_order_dtl` | 订单明细表 | 明细 | 订单行项目 | 十万级 |
...
> 直接采用真实项目数据字典的 10 列结构，**不得增删列**。每张表一张表格。
...
| 4 | `aimPackWt` | 目标件重 | - | 否 | decimal | 10,2 | Y | - | 精度,小数 |
| ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |
...
```
备注：[冗余:pmbd_customer.custName]，下单时快照，不随客户主数据变更
```
...
|--------------|----------------|-------|---------------|------|
| PMPM007 | 订单号 | `pmom_order_main` | `orderNo` | ✅ 已覆盖 |
| PMPM007 | 目标件重 | `pmom_order_main` | `aimPackWt` | ✅ 已覆盖 |
...
### DB-A 命名规范（8 项）
- [ ] A01 — 表名符合 `[领域码][模块码]_[业务含义]` 格式，全小写蛇形
- [ ] A02 — 表后缀使用固定词表（`_main`/`_dtl`/`_log`/`_resume`/`_rel`/`_cfg`）
- [ ] A03 — 字段英文名为小驼峰，且与接口/spec 可一一映射
- [ ] A04 — 无数据库保留字裸用作字段名
- [ ] A05 — 布尔/时间/金额/数量/重量字段遵循 `xxxFlag`/`xxxTime`/`xxxAmt`/`xxxQty`/`xxxWt` 后缀约定
- [ ] A06 — 普通索引命名符合 `idx_[表名去前缀]_[字段]`
- [ ] A07 — 唯一索引命名符合 `uk_[表名去前缀]_[字段]`
- [ ] A08 — 主键命名符合 `pk_[表名去前缀]`

### DB-B 系统字段（7 项）
- [ ] B01 — 每张业务表含 `id` 主键
- [ ] B02 — 含 `createdBy` + `createdTime`
- [ ] B03 — 含 `updatedBy` + `updatedTime`
- [ ] B04 — 含 `deletedFlag`（逻辑删除）
- [ ] B05 — 含 `tenantId`（单租户系统标注"暂留空"）
- [ ] B06 — 7 个系统字段统一置于字段清单末尾审计区
- [ ] B07 — 业务表无物理删除设计，查询默认带 `deletedFlag = 0`

### DB-C 主键与索引（5 项）
- [ ] C01 — 主键选型明确（雪花/UUID/自增），未用业务字段作物理主键
- [ ] C02 — 每张表有索引清单，列出索引名/类型/字段/用途
- [ ] C03 — 业务唯一键由 `uk_*` 唯一索引保证
- [ ] C04 — 外键字段、高频查询字段已建索引
- [ ] C05 — 联合索引遵循最左前缀，单表索引数 ≤ 5

### DB-D 文档完整性（5 项）
- [ ] D01 — 每个模块含固定 4 节：ER 图 / DB 清单 / 数据字典 / DDL
- [ ] D02 — ER 图含图片占位 + 实体清单（实体名/物理表/关系）
- [ ] D03 — 数据字典严格使用 10 列标准格式，列无增删
- [ ] D04 — DDL 字段带 COMMENT、表带 COMMENT，含索引定义
- [ ] D05 — 有状态流转/字段留痕需求的单据，配套设计 `_log`/`_resume` 表

### DB-E 字段类型与工程约定（4 项）
- [ ] E01 — 逻辑类型在目标数据库有明确物理映射（§5b.1），DDL 类型与之一致
- [ ] E02 — 同类业务字段精度全库统一（金额/数量/重量口径一致，无 `10,2` 与 `12,2` 混用）；变长文本 > 2000 改 `text`
- [ ] E03 — 冗余字段在备注列标注 `[冗余:来源表.字段]` 及是否随源更新
- [ ] E04 — 外键策略明确（默认逻辑外键，不建物理 FK；`FK` 列仅表关联关系）

### DB-X 与 spec 联动（5 项）⬅ 闭环核心
- [ ] X01 — **输出对象覆盖**：spec IPO Output 涉及的每个持久化对象在 DB 中有对应表
- [ ] X02 — **输入字段落库**：spec IPO 需持久化字段在 DB 对应表有对应列
- [ ] X03 — **中文名一致**：DB「字段中文名」与 spec IPO「字段名」一致
- [ ] X04 — **英文名一致**：DB「字段英文名」与接口报文「英文字段」一致
- [ ] X05 — **联动矩阵存在**：数据库分册末尾有 spec 功能编码 → DB 表/字段 的联动矩阵，无遗漏
...
修复动作：
  [B04] 已在 pmom_order_main 审计区补 deletedFlag tinyint default 0
  [X03] 已统一为"订单号"
复验：34/34 通过 → ✅ DONE
```
