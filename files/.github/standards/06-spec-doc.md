...
- [ ] P01 — 信息封面和签署封面均存在，变量字段完整
- [ ] P02 — 修订记录存在且版本、日期、修改说明完整
- [ ] P03 — 自动目录显示三级标题和页码
- [ ] P04 — 页眉符合交付 profile 且不泄露未授权信息
- [ ] P05 — 页脚包含文档代号、版本、页码和隐私级别

### B. 正文结构完整性（6 项）

- [ ] S01 — 第 1～3 章全部存在
- [ ] S02 — 系统目标为 3～5 条，每条 20～60 字；事实不足则标 Pending
- [ ] S03 — 每个子模块包含 4.x.1～4.x.6
- [ ] S04 — 最后两节为数据需求和报表设计
- [ ] S05 — 所有图片节点有统一占位或有效引用
- [ ] S06 — 目录层级与正文一致

### C. 编码规范（4 项）

- [ ] C01 — 流程编码符合 `[模块码]-A-[NN]`
- [ ] C02 — 活动编码符合 `[流程编码]-[E/C/M]-[NN]` 且连续
- [ ] C03 — 功能编码符合 `[模块码][NNN]` 或含子域码的 `[模块码][子域码][NNN]`
- [ ] C04 — 编码和稳定 ID 无重复

### D. 组织与术语（4 项）

- [ ] D01 — 角色表五列齐全
- [ ] D02 — 所有参与流程或权限的角色都已定义
- [ ] D03 — 术语表四列齐全
- [ ] D04 — 每条术语有定义、边界和来源

### E. IPO 质量（11 项）

- [ ] I01 — 每个功能有且仅有画面逻辑和 IPO 两小节
- [ ] I02 — 画面逻辑含入口、类型、原型引用和字段说明
- [ ] I02b — 有字段联动时存在完整字段交互矩阵
- [ ] I02c — 有状态变化时存在状态图和状态约束表
- [ ] I03 — 多页面功能按页面分段
- [ ] I04 — IPO 表固定五列
- [ ] I05 — 无输入项使用 `/`
- [ ] I06 — 字段行描述必填、类型、来源和校验
- [ ] I07 — 提交类操作覆盖校验、数据变化、触发事件、成功和失败
- [ ] I08 — 取消类操作明确不保存修改
- [ ] I09 — 条件必填、显隐和联动规则包含触发与还原逻辑

### F. 流程说明（3 项）

- [ ] F01 — 每条流程包含流程图、说明和活动表
- [ ] F02 — 活动表七列齐全并覆盖所有节点
- [ ] F03 — 流程与作业画面对照表覆盖所有活动

### G. 数据与报表（2 项）

- [ ] G01 — 数据需求区分外部输入和数据输出，并说明来源/目标
- [ ] G02 — 每张报表具备权限、刷新、导出、来源、查询条件、输出字段和样例占位

### H. 权限设计（2 项 + 1 条件项）

- [ ] H01 — 权限矩阵覆盖全部功能和操作
- [ ] H02 — 权限值只使用约定枚举
- [ ] H03（条件项）— 存在行级权限时，范围维度与控制字段完整；无需求时不计入总数

### X. 跨文档一致性（6 项）

- [ ] X01 — 流程清单与活动集合双向一致
- [ ] X02 — 活动集合与画面对照表双向一致
- [ ] X03 — 对照表引用的功能均有定义，且无孤儿功能
- [ ] X04 — 功能与权限矩阵双向一致
- [ ] X05 — 内部接口引用的提供方和消费方功能存在
- [ ] X06 — 流程、岗位表和权限矩阵的角色集合一致

## §十二 验证闭环

验证结果逐项记录：规则 ID、`Pass/Fail/Pending/NotApplicable`、证据路径与锚点、差异和建议。
