...
- [ ] A01 — 集成编码符合已声明格式，系统代号已登记
- [ ] A02 — 编码、稳定 ID、operationId 和 schema 名称唯一
- [ ] A03 — HTTP URI 使用稳定资源语义且符合 versioning profile
- [ ] A04 — HTTP 方法、安全性、幂等性和状态码语义一致
- [ ] A05 — 集成 KIND/type 码已在词典注册，无临时自造值
- [ ] A06 — 接口名称和 operationId 清晰、稳定、无实现泄露

### IF-B 契约完整性（11 项）

- [ ] B01 — 集成接口六项触发与投递语义完整
- [ ] B02 — 请求、成功响应和错误响应 schema 齐全
- [ ] B03 — 字段表包含稳定 ID、名称、逻辑类型、必填/可空和约束
- [ ] B04 — 使用 OpenAPI/JSON Schema 逻辑类型，不使用语言专属类型
- [ ] B05 — 响应和错误模型已在 profile 声明，文档、示例和 OpenAPI 一致
- [ ] B06 — 请求与响应合成示例语法有效且符合 schema
- [ ] B07 — required、nullable/可空、缺失和默认值语义明确
- [ ] B08 — object/array/引用层级与 schema 和示例一致
- [ ] B09 — 列表接口符合已声明分页、排序和过滤 profile
- [ ] B10 — 日期时间、布尔、枚举、小数、标识符和空值符合序列化 profile
- [ ] B11 — 批量接口明确原子性/部分成功、限制、单项映射和重试单位

### IF-C 安全与可靠性（5 项）

- [ ] C01 — 认证、授权和 security scheme 明确且一致
- [ ] C02 — 传输保护以及按威胁模型需要的签名/防重放已说明
- [ ] C03 — 敏感字段的最小化、脱敏、日志和导出策略明确
- [ ] C04 — 写操作逐一声明幂等与并发冲突策略
- [ ] C05 — 超时、重试、退避、补偿和人工恢复策略完整

### IF-D 文档与演进（8 项）

- [ ] D01 — 元信息、契约、示例、安全、可靠性和追溯章节齐全
- [ ] D02 — 接口清单含稳定 ID、类型、协议、提供/消费方和关联 ID
- [ ] D03 — 错误清单含 HTTP 状态、稳定标识、触发和处理建议
- [ ] D04 — 错误标识符合已声明 profile 且唯一
- [ ] D05 — 协议、版本、序列化和内容类型明确
- [ ] D06 — 数据通道使用已登记源/目标代号
- [ ] D07 — 频次、容量、顺序和投递语义明确
- [ ] D08 — OpenAPI profile 下契约可解析，operationId 唯一且 `$ref` 有效

### IF-X 跨文档一致性（8 项）

- [ ] X01 — spec 中明确需服务端接口的功能均有 API 承载
- [ ] X02 — 每个 API 都能追溯到功能、集成事件或有证据的架构能力
- [ ] X03 — 持久化接口字段 ID 能映射到词典和 DB 字段；例外有说明
- [ ] X04 — 接口字段中文语义与 spec/词典一致
- [ ] X05 — 接口逻辑类型与词典/DB 逻辑类型兼容
- [ ] X06 — API/消息触发条件能追溯到 IPO 或集成事件
- [ ] X07 — 接口清单的功能、字段和数据落点引用存在
- [ ] X08 — API 稳定 ID、编码和 operationId 集合无重复
