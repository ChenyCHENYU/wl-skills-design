...
读取 `.github/standards/04-api-design.md`，作为验证基准。**唯一权威检查清单为 §十（35 项，IF-A/B/C/D/X 五组）**；IF-X 集合比对算法见 §十一。
...
按 §十二 优先级（IF-X → IF-A → IF-C → IF-B → IF-D）用 `replace_string_in_file` 修复，修复后复验全部 35 项。
