...
- 验证：默认只读。先运行 `wl-skills-design verify spec/flowchart` 获取 [M] 机械结论，再由 Agent 判 [J] 项，合并输出规则编号报告。
- 修复：必须明确提出“修复”或同等授权；修改前先说明范围。
- 评审/影响分析：默认只读，只在要求保存报告时创建报告文件。

## 从零到一

没有既有文档时，按 [输入准备清单](./inputs-checklist.md) 分层提供：最小启动（项目代号/模块范围/业务目标）→ 业务事实（岗位/对象/状态流/操作规则/报表）→ 技术画像（数据库与接口策略）。不必一次备齐，缺失项自动标 Pending 并生成关键问题清单。
