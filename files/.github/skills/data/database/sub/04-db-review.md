...
> 规范来源：`standards/03-database.md §八（30 项）/§九（集合比对）/§十（修复协议）`。
> 本文件**不自行定义检查项**，所有检查以 §八 为准，避免双源脱节。

## 目标

对一份数据库设计执行 30 项验证，自动修复失败项，输出标准化报告。

## 步骤

### Step 1 — 加载规范

读取 `standards/03-database.md`，以 §八 的 30 项（DB-A/B/C/D/X）为唯一检查清单。
...
DB-X → DB-B → DB-C → DB-A → DB-D。修复后**必须复验**全部 30 项。
