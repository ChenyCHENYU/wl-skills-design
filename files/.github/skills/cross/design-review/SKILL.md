...
| 模板 | 用途 |
|------|------|
| `templates/review-report.md` | 评审报告**空白模板**（仪表盘 + P0 清单 + 追溯矩阵 + 修复任务，`{占位符}`，直接套用）|

> **空白模板 vs 真实样例**：`templates/review-report.md` 是空白起点；
> `examples/01-review-report.md` 是**真实样例（质量标杆）**——订单管理模块第 1 轮评审完整报告，
> 演示 P0 一票否决、暂挂排除分母、追溯矩阵，生成结果须**对照样例自检且不低于它**。
