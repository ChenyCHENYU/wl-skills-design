...
- **创建**：`/create-glossary` → 建立字段中英文名/枚举/编码的统一词典（建议设计开始前先建骨架）
- **验证**：`/validate-glossary` → 校验 spec/数据库/接口字段是否都 ⊆ 词典，揪出词典外字段
  - 让字段对不齐从「评审时发现」提前到「设计时杜绝」
  - 集成评审 D4 的字段/枚举一致性以词典为比对基准（O(N)）
...
```bash
# 创建规范文件（当前已生 01/03/04/06/07/08，下一个是 09）
# 02/05 为规划中 stub
